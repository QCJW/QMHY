...
// ==================== ASMR 按需加载 ====================
if (isset($_GET['asmr']) || isset($_GET['w']) || strpos($_SERVER['REQUEST_URI'], '?asmr=') !== false) {
    require_once dirname(__FILE__) . '/lib/asmr.php';
}
...
function themeFields($layout)
{
    if (strpos($_SERVER['SCRIPT_NAME'], "write-post.php")) {
        $okdizhi = new Typecho_Widget_Helper_Form_Element_Text('okdizhi', NULL, NULL, _t('豆瓣/Bangumi'), _t('在这里填入豆瓣或bangumi的栏目id然后点击右侧相应按钮即可获取影视信息<span class="right"><button type="button" id="douban" class="btn primary" style="margin-right: 10px;">豆瓣</button><button type="button" id="bangumi" class="btn primary">Bangumi</button></span>'));
        $okdizhi->setAttribute('id', 'okdizhi');
        ...
        $icons = new Typecho_Widget_Helper_Form_Element_Text('icons', NULL, NULL, _t('图标名字'), _t('菜单导航文字左侧图标参数，网站支持图标请访问<a href="https://store.typecho.work/demo/unicons2.1.9/" target="_blank">https://store.typecho.work/demo/unicons2.1.9/</a>去除uil-就是图标的名字'));
        $icons->input->setAttribute('class', 'text w-100');
...
function tx($mail, $re = 0, $id = 0)
{
    ...
    if (strstr($c, "qq.com") && is_numeric($f) && strlen($f) < 11 && strlen($f) > 4) {
        ...
        if ($id > 0) {
            $g = Helper::options()->rootUrl . '/about.html?id=' . $id . '" data-type="qqtx';
        }
    } else {
        $g = $b . $d . '?d=mm';
    }
...
function themeInit($archive)
{
    ...
    if ($archive->request->isPost() && $archive->request->likeup && $archive->request->do_action) {
        ...
        $security->protect();
        likeup(intval($archive->request->likeup), $archive->request->do_action);
        exit;
    }
    ...
    if ($archive->request->isPost() && $archive->request->collect) {
        ...
        collect(intval($archive->request->collect), $user->uid);
        exit;
    }
...
function ajaxComment($archive)
{
    ...
    if (!$user->pass('editor', true) && $archive->authorId != $user->uid && $options->commentsPostIntervalEnable) {
    ...
    $sf = '';
    if ($user->uid > 0 && $user->uid == $archive->authorId) {
...
class Widget_Post_tongleisuiji extends Widget_Abstract_Contents
{
    ...
    public function execute()
    {
        ...
        if ($this->parameter->mid > 0) {
            $select->where('table.relationships.mid = ?', intval($this->parameter->mid));
        }

        $select->where('table.contents.cid <> ?', intval($this->parameter->cid))
            ->where("table.contents.password IS NULL OR table.contents.password = ''")
            ->where('table.contents.type = ?', 'post')
            ->limit(intval($this->parameter->pageSize));

        // 使用随机偏移替代 ORDER BY RAND()
        $maxId = $this->db->fetchRow($this->db->select('MAX(cid) as max')->from('table.contents'));
        if ($maxId && $maxId['max'] > 0) {
            $pageSize = intval($this->parameter->pageSize);
            $max = $maxId['max'];
            $randOffset = mt_rand(1, max(1, $max - $pageSize));
            $select->where('table.contents.cid >= ?', $randOffset)->order('table.contents.cid');
        } else {
            $select->order('RAND()'); // fallback
        }
...
{
    // paopao 表情（::(表情)）
    $paopao = array('呵呵', '哈哈', '吐舌', '太开心', '笑眼', '花心', '小乖', '乖', '捂嘴笑', '滑稽', '你懂的', '不高兴', '怒', '汗', '黑线', '泪', '真棒', '喷', '惊哭', '阴险', '鄙视', '酷', '啊', '狂汗', 'what', '疑问', '酸爽', '呀咩爹', '委屈', '惊讶', '睡觉', '笑尿', '挖鼻', '吐', '犀利', '小红脸', '懒得理', '勉强', '爱心', '心碎', '玫瑰', '礼物', '彩虹', '太阳', '星星月亮', '钱币', '茶杯', '蛋糕', '大拇指', '胜利', 'haha', 'OK', '沙发', '手纸', '香蕉', '便便', '药丸', '红领巾', '蜡烛', '音乐', '灯泡', '开心', '钱', '咦', '呼', '冷', '生气', '弱', '吐血');
    $content = preg_replace_callback('/\:\:\(\s*(' . implode('|', $paopao) . ')\s*\)/is', function ($m) {
        return '<img class="biaoqing" src="' . theurl . '/assets/owo/paopao/' . urlencode($m[1]) . '_2x.png">';
    }, $content);

    // aru 表情（:@(表情)）
    $aru = array('高兴', '小怒', '脸红', '内伤', '装大款', '赞一个', '害羞', '汗', '吐血倒地', '深思', '不高兴', '无语', '亲亲', '口水', '尴尬', '中指', '想一想', '哭泣', '便便', '献花', '皱眉', '傻笑', '狂汗', '吐', '喷水', '看不见', '鼓掌', '阴暗', '长草', '献黄瓜', '邪恶', '期待', '得意', '吐舌', '喷血', '无所谓', '观察', '暗地观察', '肿包', '中枪', '大囧', '呲牙', '抠鼻', '不说话', '咽气', '欢呼', '锁眉', '蜡烛', '坐等', '击掌', '惊喜', '喜极而泣', '抽烟', '不出所料', '愤怒', '无奈', '黑线', '投降', '看热闹', '扇耳光', '小眼睛', '中刀');
    $content = preg_replace_callback('/\:\@\(\s*(' . implode('|', $aru) . ')\s*\)/is', function ($m) {
        return '<img class="biaoqing" src="' . theurl . '/assets/owo/aru/' . urlencode($m[1]) . '_2x.png">';
    }, $content);
...
{
    $db = Typecho_Db::get();
    $prow = $db->fetchRow($db->select('parent')->from('table.comments')->where('coid = ?', intval($coid)));
    $parent = $prow['parent'];
    if ($parent != "0") {
        $arow = $db->fetchRow($db->select('author')->from('table.comments')->where('coid = ? AND status = ?', intval($parent), 'approved'));
        if ($arow['author']) {
            echo '<a href="#comment-' . $parent . '">@' . $arow['author'] . '</a>';
        } else {
            echo '';
        }
    } else {
...
function timesince($older_date, $comment_date = false)
{
    ...
    $newer_date = time();
    $since = abs($newer_date - intval($older_date));
...
{
    $db = Typecho_Db::get();
    $userinfo = $db->fetchRow($db->select()->from('table.users')->where('table.users.uid = ?', intval($id)));
    return $userinfo;
}
    ...
    if (in_array($cid, $sc)) {
        if ($k == 0) {
            if (count($sc) < 4) {
                ...
                $pcid = ',' . $cid . ',';
                $new = str_replace($pcid, ",", $row['collect']);
                $db->query($db->update('table.users')->rows(array('collect' => $new))->where('uid = ?', $uid));
            }
            echo 'ko';
        }
        return 'ko';
...
function likeup($ccid, $kg)
{
    ...
    $num = intval($row['likes']);

    if ($kg == "do") {
        $db->query($db->update('table.contents')->rows(array('likes' => $num + 1))->where('cid = ?', $cid));
        $num++;
    }
    if ($kg == "undo" && $num > 0) {
        $db->query($db->update('table.contents')->rows(array('likes' => $num - 1))->where('cid = ?', $cid));
...
function fanbiao($new)
{
    ...
    if (isset($spurl) && strpos($spurl, '$') === false) {
        $spurl = '全集$' . $spurl;
    }

    if (isset($spurl)) {
        $string_arr = array_filter(explode("\r\n", $spurl));
        $num = count($string_arr);
        $numx = $num;
        $jiend = array_filter(explode('$', $string_arr[$num - 1]))[0];
...
class Widget_Post_fanjubiao extends Widget_Abstract_Contents
{
    ...
    public function execute()
    {
        ...
        if ($this->parameter->fanjubiao) {
            ...
            $select->where('table.contents.cid in ?', array_map('intval', $fanju));
        }
        $this->db->fetchAll($select, array($this, 'push'));
    }
}
...
class Widget_Post_cat extends Widget_Abstract_Metas
{
    ...
    public function execute()
    {
        ...
        $select = $this->select()->from($prefix . 'metas')
            ->where('table.metas.parent = ?', intval($this->parameter->mid))
